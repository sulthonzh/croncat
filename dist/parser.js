...
function resolveToken(token, fieldIndex) {
    const lower = token.toLowerCase();
...
function expandField(field, fieldIndex) {
...
function validateField(values, fieldIndex) {
    const [min, max] = RANGES[fieldIndex];
...
function describe(values, fieldIndex) {
    const names = ['minute', 'hour', 'day of month', 'month', 'day of week'];
    const [min, max] = RANGES[fieldIndex];
...
function parseCrontab(content) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (!raw || raw.startsWith('#'))
            continue;
        if (raw.includes('=') && !raw.match(/^\S+\s+\S+\s+\S+\s+\S+\s+\S+/))
            continue;
        ...
        try {
            ...
            if (errors.some(e => e.lineIndex === lineIndex))
                continue;
            entries.push({
...
// ── Next runs ────────────────────────────────────────────────────────────
function getNextRuns(entry, count = 5, from) {
    ...
    while (runs.length < count && iterations < maxIterations) {
        ...
        if (entry.fields.minute.includes(m) &&
...
function formatTable(entries) {
    ...
    lines.push('Schedule'.padEnd(schedW) + '  ' +
    ...
    for (const e of entries) {
        ...
        lines.push(e.schedule.padEnd(schedW) + '  ' +
            cmd.padEnd(40) + '  ' +
            e.description);
    }
    return lines.join('\n');
}
function formatJSON(entries, errors, conflicts) {
    return {
